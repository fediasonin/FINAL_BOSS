...
class GraphProcessor:
    ...
    def process_leaf_node(self, item, parent_data):
        ...
        # Формирование комментария с учетом родительских данных и текущего узла
        ...
        self.api_client.create_variable(
            name=item['Правило'],  # Название переменной — "Правило"
            var_type='rule',  # Тип переменной — например, "rule"
            value=item['Описание'],  # Значение переменной — "Описание"
            comment=comment  # Комбинированный комментарий
        )

    def process_empty_node(self):
        pass
        ...
        if 'Группа PT-NAD' in table.columns:
            ...
            for _, item in table.iterrows():
                ...
                if os.path.exists(child_filepath):
                    ...
                    self.executor.submit(self.process_empty_node, item)
                else:
                    self.executor.submit(self.process_leaf_node, item, parent_data)

            def all_children_done(fut):
                logging.info(f"Обрабатываю узел: {parent_data}")
                #Код того, как мы отправляем не конечную группу (не лист)
            ...
            all_children_future.add_done_callback(all_children_done)
...
if __name__ == "__main__":
    BASE_DIRECTORY = r"Z:\PT-NAD_Catalog"
    api_client = APIClient('https://172.16.0.88/api/v2', 'api-user', 'QWer1234$')
